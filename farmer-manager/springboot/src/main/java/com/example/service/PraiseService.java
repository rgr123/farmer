...
public class PraiseService {
    ...
    public void add(Praise praise) {
        ...
        if (ObjectUtil.isNotEmpty(policy)) {
            // 处理 policy.getNum() 可能为 null 的情况
            int currentNum = (policy.getNum() != null) ? policy.getNum() : 0;
            policy.setNum(currentNum + num);
            policyMapper.updateById(policy);
        } else {
            throw new RuntimeException("Policy not found for ID: " + praise.getPolicyId());
        }
    }
